  ...
  return (
    <ScrollReveal delay={delay} className="h-full">
      <div className="group relative rounded-2xl overflow-hidden transition-all duration-700 hover:-translate-y-1 h-full">
        ...
        <div className="absolute inset-0 rounded-2xl bg-gradient-to-br from-accent/30 via-accent/10 to-accent/20 opacity-0 group-hover:opacity-100 transition-opacity duration-700 -z-10 blur-[1px] scale-[1.01]" />

        <div className="relative h-full bg-surface-2 rounded-2xl border border-border group-hover:border-accent/20 transition-colors duration-500 overflow-hidden">
          {/* Image container */}
                ...
                <div>
                  <div className="flex items-center gap-2 mb-1">
                    <div className="w-7 h-7 rounded-full bg-accent/20 border border-accent/30 flex items-center justify-center">
                      <span className="text-accent font-heading font-bold text-xs">{item.name.charAt(0)}</span>
                    </div>
                    <p className="font-heading font-semibold text-white text-sm">{item.name}</p>
                  </div>
                  {item.value && (
                    <p className="text-green font-heading font-bold text-2xl tracking-tight ml-9">{item.value}</p>
                  )}
                </div>
                <div className="w-9 h-9 rounded-full bg-white/5 border border-white/10 flex items-center justify-center shrink-0 group-hover:bg-accent/15 group-hover:border-accent/30 transition-all duration-500">
                  <svg className="w-4 h-4 text-white/50 group-hover:text-accent transition-colors" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth="2">
...
export default function Results() {
  return (
    <section id="results" className="py-24 md:py-32 bg-surface relative overflow-hidden">
      {/* Ambient light */}
      <div className="absolute top-0 right-1/4 w-[500px] h-[500px] bg-accent/4 rounded-full blur-3xl" />
        ...
        <ScrollReveal>
          <div className="flex flex-col items-center mb-4">
            <div className="inline-flex items-center gap-2 glass-card rounded-full px-4 py-1.5 mb-6">
              ...
              <span className="text-accent text-xs font-semibold uppercase tracking-widest">Resultados reais</span>
            </div>
